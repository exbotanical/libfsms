...
void fsm_create_test() {
  ...
  cmp_ok(array_size(fsm->states), "==", 0,
...
void fsm_state_create_test() {
  ...
  cmp_ok(array_size(s->transitions), "==", 0,
  ...
  is(((StateDescriptor*)array_get(fsm->states, -1))->name, OFF_STATE,
...
void conditional_transition_test() {
  ...
  fsm_transition_create(fsm, TRANSITION_NAME, off_s, on_s, cond_false,
                        on_action_handler);
  fsm_transition_create(fsm, TRANSITION_NAME, on_s, off_s, cond_false,
                        off_action_handler);
...
void with_context_test() {
  ...
  fsm_transition_create(fsm, TRANSITION_NAME, off_s, on_s, cond_context,
                        on_action_handler);
  fsm_transition_create(fsm, TRANSITION_NAME, on_s, off_s, cond_context,
                        off_action_handler);
